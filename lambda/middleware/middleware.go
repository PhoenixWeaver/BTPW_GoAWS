...
- Returns proper HTTP responses for authentication failures
 ...
 Section Order Analysis (REORDERED BY CALL FLOW):
  1. extractTokenFromHeaders (utility): Token extraction utility function
   ↓ calls
  2. parseToken (utility): JWT parsing and validation utility function
   ↓ calls
  3. ValidateJWTMiddleware (main function): Main middleware function with extensive documentation
*/
...
// !SECTION 1 - extractTokenFromHeaders
...
// !SECTION 2 - parseToken - JWT PARSING AND  validated claims
...
func parseToken(tokenString string) (jwt.MapClaims, error) {
	...
	claims, ok := token.Claims.(jwt.MapClaims) // from types.go - CreateToken() function we can see that the claims are of type jwt.MapClaims
	if !ok {
...
// !SECTION 3 - ValidateJWTMiddleware
...
2. Middleware Functions (REORDERED BY CALL FLOW):
   - extractTokenFromHeaders: Token extraction utility (FIRST)
   - parseToken: JWT parsing and validation (SECOND)
   - ValidateJWTMiddleware: Main middleware wrapper function (THIRD)
